...
state = [0x01 0x02 0x03 0x04;...   
    ...
    %matricex indexing(row,col)
        ...
        row1 = bitshift(val, -4)+1;
        
        %0x0F in binary is 00001111 and by using and gate we extract
        %rightmost 4 bits as the rest is set to 0 
        col1 = bitand(val,0x0F)+1;
        
        
       
        %get a message from Sbox 
        StateTest(row,col) = Sbox(row1,col1);
        disp(['Original Message:',dec2hex(state(row,col))]);
        disp(['SubByte:', dec2hex(StateTest(row,col))]);
